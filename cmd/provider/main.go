...
import (
	...
	apisCluster "github.com/mikegio27/provider-jfrog-project/apis/cluster"
	apisNamespaced "github.com/mikegio27/provider-jfrog-project/apis/namespaced"
	"github.com/mikegio27/provider-jfrog-project/config"
	"github.com/mikegio27/provider-jfrog-project/internal/clients"
	controllerCluster "github.com/mikegio27/provider-jfrog-project/internal/controller/cluster"
	controllerNamespaced "github.com/mikegio27/provider-jfrog-project/internal/controller/namespaced"
	"github.com/mikegio27/provider-jfrog-project/internal/features"
	"github.com/mikegio27/provider-jfrog-project/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for jfrog-project").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-jfrog-project"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-jfrog-project",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped jfrog-project APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced jfrog-project APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped jfrog-project controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced jfrog-project controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped jfrog-project controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced jfrog-project controllers")
	}
